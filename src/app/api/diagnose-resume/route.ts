...
2. \`red_flags\` (字符串数组): 找出 2-3 个结构性问题。**必须引用简历中的具体经历**（用公司名或职位名定位），指出该段经历存在什么问题。例如："在[公司名]的经历中，仅罗列日常职责，无任何量化成果，流水账严重"。每条 20-40 字。
3. \`structural_advice\` (字符串数组): 给出 3 条修改建议。**必须明确指出哪段经历需要怎么改**，例如："将[公司名]的工作描述从职责罗列改为 3 条量化成果：主导XX项目提升XX%"。每条 25-50 字。
