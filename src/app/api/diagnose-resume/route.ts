  ...
  "red_flags": [
    {
      ...
      "issue": "这句话的问题，15-30字",
      "suggestion": "改写建议：先说明问题原因（10字），再给出具体的改写示例（50-80字），示例要包含量化数据、动作动词、业务价值"
    }
  ],
  "structural_advice": [
    {
      ...
      "suggestion": "结构调整建议：先说明调整方向（10字），再给出具体的改写示例（80-120字），示例要展示完整的STAR结构（情境-任务-行动-结果），包含具体数据和业务影响"
    }
  ]
}
...
- suggestion 字段必须包含具体的改写示例，不能只说"建议量化"或"建议补充数据"，要直接写出改好的句子
- 示例中要包含具体的数字（如"提升15%"、"覆盖200+用户"）、动作动词（如"主导"、"重构"、"优化"）、业务价值（如"降低成本"、"提升效率"）
- 语言犀利直接，不要客套
- 只输出 JSON，不要任何解释`;
